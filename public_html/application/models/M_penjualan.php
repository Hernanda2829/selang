...
class M_penjualan extends CI_Model{
	...
	function simpan_penjualan($nofak,$total,$jml_uang,$kembalian,$sisakurang,$customer,$nota,$carabayar,$statusbayar,$prdbln,$tgljtempo){
		...
		try {
			...
			foreach ($this->cart->contents() as $item) {
				$data=array(
					'd_jual_nofak' 			=>	$nofak,
					'd_jual_tanggal' 		=>  date('Y-m-d H:i:s'),
			...
			//if ($carabayar==="Tempo" && floatval($jml_uang) > 0) { //perintah ini mengharuskan ada dp
			if ($carabayar==="Tempo") {
				...
				$data_bayar = array(
					'bayar_nofak' 		=> $nofak,
					'bayar_tanggal' 	=> date('Y-m-d H:i:s'),
					'bayar_total' 		=> $total,
	...
	function cek_customer($custid){
		$query = 'a.jual_nofak,b.stop_sales';
		$this->db->select($query);
		$this->db->from('tbl_jual a');
		$this->db->join('kolek b', 'b.kol_bln = a.jual_kolek', 'left');
		$this->db->where('a.jual_bayar_status', "Belum Lunas");
		$this->db->where('a.jual_cust_id', $custid);
