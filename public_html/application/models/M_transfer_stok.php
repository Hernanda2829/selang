...
class M_transfer_stok extends CI_Model{
	...
	function tampil_transtok_history() {
		$coid = $this->session->userdata('coid');
		$query = $this->db
					->select('a.trans_stok_no, a.trans_stok_tgl, a.tgl_konfirm, a.proses_konfirm, a.trans_reg_id, a.regid_terima, a.total_selisih, pengirim.reg_name AS cabang_pengirim, penerima.reg_name AS cabang_penerima')
					->from('tbl_transtok a')
					->join('regions AS pengirim', 'a.trans_reg_id = pengirim.reg_id')
					->join('regions AS penerima', 'a.regid_terima = penerima.reg_id')
					->where('a.trans_co_id', $coid)
					->order_by('DATE(a.trans_stok_tgl)', 'DESC');
		$result = $query->get();
		return $result;
	}
