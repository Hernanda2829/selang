...
class Penjualan_admin extends CI_Controller{
	function __construct(){
		...
		$this->load->model('M_penjualan_admin');
	...
	function index(){
		if($this->session->userdata('akses')=='1') {
			$regid = $this->session->userdata('cabses'); //mengambil session cabang
			if (!empty($regid)) {
				$data['data']=$this->M_penjualan_admin->tampil_barang2($regid);
			}else {
				$data['data']=$this->M_penjualan_admin->tampil_barang();
			}
			$data['periode'] = $this->M_penjualan_admin->tampil_periode();
			$data['userid']=$this->Mlogin->tampil_user();
			$data['cust']=$this->M_penjualan_admin->tampil_customer_all();
			$data['tglskrg']=date('Y-m-d');
			$this->load->view('admin/v_penjualan_admin',$data);
		}
	}
	
	function tampil_stok() {
		$regid=$this->input->post('regid');
        $data = $this->M_penjualan_admin->tampil_stok($regid);
        if (!empty($data)) {
	...
	function get_barang(){
		if($this->session->userdata('akses')=='1') {
			...
			$regid=$this->input->post('id_reg');
			$x['brg']=$this->M_penjualan_admin->get_produk($kobar,$regid);
			if ($x['brg']->num_rows() > 0) {
				$data_barang = $x['brg']->row_array();
				$disc_id = $data_barang['barang_disc_id'];
				$x['disc']=$this->M_penjualan_admin->get_discount($disc_id);
			}	
			$this->load->view('admin/v_detail_barang_jual_admin',$x);
		}
	}
	...
	function update_set() {
		...
		redirect('admin/penjualan_admin');
	}
	...
	if($this->session->userdata('akses')=='1') {
		...
		$this->session->set_userdata('tglses',$tgljual);
		$this->session->set_userdata('cabses',$cab);
		$this->session->set_userdata('nmcabses',$nmcab);
		...
		$produk = $this->M_penjualan_admin->get_barang($kobar);
		$jstok = (float) $this->input->post('stok');
		...
		if ($carabayar==="Cash") {
			if(!empty($total) && !empty($jml_uang)){
				...
				if($jml_uang < $total){
					...
					$nofak=$this->M_penjualan_admin->get_nofak();
					$this->session->set_userdata('nofak',$nofak);	//dibuat session untuk langsung tampil v_faktur, model -> cetak_faktur
					$tgljual=$this->session->userdata('tglses');
					$cab=$this->session->userdata('cabses');
					...
					$prdbln='';
					$tgljtempo='';
					$order_proses=$this->M_penjualan_admin->simpan_penjualan($nofak,$tgljual,$cab,$total,$jml_uang,$kembalian,$sisakurang,$customer,$nota,$carabayar,$statusbayar,$prdbln,$tgljtempo);
					if($order_proses){
						$this->cart->destroy();
						$this->session->unset_userdata('tglses');
						$this->session->unset_userdata('cabses');
						$this->session->unset_userdata('nmcabses');
						...
						$this->load->view('admin/alert/alert_sukses_admin',$data);	
					}else{
						redirect('admin/penjualan_admin');
					}
					//batas simpan------------------//
	...
	function cetak_faktur(){
		...
		$x['data']=$this->M_penjualan_admin->cetak_faktur();
	...
	function tampil_customer() {
		$regid = $this->input->post('regid');
		$data = $this->M_penjualan_admin->tampil_customer($regid);
		if (!empty($data)) {
            echo json_encode($data);
        } else {
            echo json_encode(array('error' => 'Data not found'));
