...
<head>
    ...
    $dn="display:none;";
    $sess_cab = $this->session->userdata('cabses');
    $sess_nmcab = $this->session->userdata('nmcabses');
    if (!empty($sess_cab)) {
        $cabdiv="display:none;";
        $nmcabdiv="";
    }else {
        $nmcabdiv="display:none;";
    }
    ?>  
...
<body>
   ...
   ?>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <center><?php echo $this->session->flashdata('msg');?></center>
                <h3 class="page-header">Transaksi
                    <small>Penjualan - <?php echo $userid['reg_name'];?></small>
                    <a class="pull-right btn-cariproduk" href="#" data-toggle="modal"><small>Cari Produk!</small></a>
                </h3> 
            </div>
        </div>
        <!-- /.row -->
        ...
        <div class="row">
            <div class="col-lg-12">
            <form action="<?php echo base_url().'admin/penjualan_admin/add_to_cart'?>" method="post">
            <table>
            <tr>
                <th style="width:100px;vertical-align:middle;padding-bottom:10px;">Tanggal</th>
                <td style="padding-bottom:10px;">
                    <div class='input-group date' id='datepicker2' style="width:200px;">
                        <input type='text' name="tgljual" id="tgljual" class="form-control" value="<?php echo $this->session->userdata('tglses');?>" placeholder="Tanggal..." required/>
                ...
                <th style="width:100px;vertical-align:middle;padding-left:10px;padding-bottom:10px;">Pilih Cabang</th>
                <td style="width:250px;padding-bottom:10px;padding-left:15px;">
                    <input type="hidden" name="txtregid" id="txtregid" value="<?= $sess_cab;?>">
                    <div id="divcab" style="<?php echo $cabdiv;?>"> 
                        <?php 
                            echo '<select name="cabang" id="cabang" class="selectpicker show-tick form-control" data-live-search="true" title="Pilih Cabang" data-width="250px">';
                    ...
                    <div id="div_nmcab" style="<?php echo $nmcabdiv;?>">
                            <input type="text" name="nmcab" id="nmcab" value="<?= $sess_nmcab;?>" class="form-control" style="width:250px" readonly>
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td style="width:20px;"></td>
                <th style="width:100px;vertical-align:middle;padding-left:10px;padding-bottom:10px;">No Nota</th>
                <td style="width:250px;padding-bottom:10px;padding-left:15px;">
            ...
            <table>
                ...
                <tr>
                    <input type="text" name="id_reg" id="id_reg" class="form-control input-sm" style="width:50px;display:none;" value="<?php echo $this->session->userdata('cabses');?>">
            ...
            <table id="tbl_data" class="table table-bordered table-condensed" style="font-size:11px;margin-top:10px;">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align:center;"><a href="<?php echo base_url().'admin/penjualan_admin/remove/'.$items['rowid'];?>" class="btn btn-warning btn-xs"><span class="fa fa-close"></span> Batal</a></td>
                    </tr>    
                    <?php $i++; ?>
                    <?php endforeach; ?>
                </tbody>
            ...
            <table id="tbl_set" style="margin-top:20px;">
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="width:20px;vertical-align:middle;padding-right:10px;"><input type="text" id="desk_set" name="desk_set" class="form-control input-sm" style="width:500px;" placeholder="Satu Set..." maxlength="150" title="Isikan Keterangan Set Paket Barang untuk tampilan Pencetakan"></td>
                        <td style="width:15px;vertical-align:middle;padding-right:10px;"><input type="text" id="jml_set" name="jml_set" class="form-control input-sm" style="width:100px;" placeholder="1 Set" maxlength="11" title="Isikan Jumlah/Quantity Set Paket Barang untuk tampilan Pencetakan"></td>
            ...
            <form action="<?php echo base_url().'admin/penjualan_admin/simpan_penjualan'?>" method="post">
            <table>
                <tr>
                    ...
                    <a href="<?php echo base_url().'admin/penjualan_admin/batalsimpan'?>" class="btn btn-danger btn-lg" title="Batalkan Transaksi Penjualan"><span class="fa fa-close"></span> Batal</a></td>
        ...
        <div class="modal fade" id="largeModal" tabindex="-1" role="dialog" aria-labelledby="largeModal" aria-hidden="true">
            ...
            <div class="modal-header">
                ...
                <h3 class="modal-title" id="myModalLabel">Data Barang - <small><span id="judul"> </span></small></h3>
            </div>
...
$(document).ready(function () {
    ...
    function handleKodeChange() {
        ...
        var idreg = { id_reg: $('#txtregid').val() };
        var postData = Object.assign({}, kobar, idreg);

        $.ajax({
            type: "POST",
            url: "<?php echo base_url().'admin/penjualan_admin/get_barang';?>",
            data: postData,
...
<script type="text/javascript"> 
    ...
    $('#tgljtempo').on("blur", function() {
        ...
        if (tglJtempoValue !== "" && tglJtempoValue < tglnowValue) {
            ...
            alert('Tanggal Jatuh Tempo harus lebih dari tanggal transaksi');
        }
    });
</script>
...
$(document).on('change', '#cabang', function(e) {
    $('#customer').selectpicker('val', '');
    var regid = $(this).val();
    $('#txtregid').val(regid);
    var nama_cab = $('#cabang option:selected').text();
    $('#nmcab').val(nama_cab);
    $.ajax({
        url: "<?php echo base_url().'admin/penjualan_admin/tampil_customer';?>",
        type: "POST",
        data: {regid: regid},
        success: function (data) {
            var parsedData = JSON.parse(data);
            if (parsedData.length !== 0) {
                var selectElement = document.getElementById('customer');      // Ambil elemen select
                selectElement.innerHTML = '';   // Bersihkan opsi sebelum menambahkan yang baru
                ...
                // Refresh selectpicker setelah mengubah opsi
...
<script>
    ...
    $('.btn-set').click(function () {
        ...
        if (checkedRows.length >= 2) {
            ...
            $.ajax({
                type: 'POST',
                url: "<?php echo base_url().'admin/penjualan_admin/update_set';?>",
...
$(document).on('click', '.btn-cariproduk', function() {
    ...
    const judul = $('#cabang option:selected').text();
    $('#judul').text(judul);
    if (regid == "") {
        alert('Pilih Cabang terlebih dahulu,');
        setTimeout(function() {
            $("#cabang").focus(); // Tambahkan delay untuk memastikan pengaturan fokus berhasil
        }, 0);
    } else if (idreg == "") {
        $('#mydata tbody').empty(); // Kosongkan isi tabel sebelum menambahkan data baru
        $('#largeModal').modal('show');
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('admin/penjualan_admin/tampil_stok'); ?>",
            data: { regid: regid },
            dataType: 'json',
            success: function (data) {
                console.log(regid);
                if (data.length === 0) {
                    ...
                    $.each(data, function (index, item) {
                        ...
                        var newRow = '<tr>' +
                            ...
                            '<td style="text-align:center;">' + item.barang_nama + '</td>' +
                            '<td style="text-align:center;">' + item.barang_disc_id + '</td>' +
                            '<td style="text-align:center;">' + item.barang_satuan + '</td>' +
                            '<td style="text-align:right;">' + harjul.toLocaleString('id-ID') + '</td>' +
                            '<td style="text-align:center;">' + formatted_stok + '</td>' +
                            '<td style="text-align:center;">' +
                                '<button type="submit" class="btn btn-xs btn-info" title="Pilih" onclick="pilihKode(\'' + item.barang_id + '\')"><span class="fa fa-edit"></span> Pilih</button>' +
                            '</td>'
                            '</tr>';
                        $('#mydata tbody').append(newRow);    
                    });
                    $('#mydata').DataTable(); // Inisialisasi DataTable setelah tabel diperbarui
                } 
            },
            error: function(xhr, status, error) {
                //console.log(error);
